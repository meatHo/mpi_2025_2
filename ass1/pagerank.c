...
int main(int argc, char** argv) {
    ...
    for (it = 0; it < P.maxit &&!converged; ++it) {
         ...
         * TODO: PageRank 계산 반복
         */
        double jump = (1.0 - d) * one_over_N;
        double dang_term = d * global_dang_mass * one_over_N;
        if (strcmp(P.gtype, "ring") == 0) {
            for (long long i = 0; i < locN; ++i) {
                long long cur = off + i; //전역 노드 중 숫자
                long long before = (cur + P.N - 1) % P.N;//이전 노드
                double contrib = pagerank_global[before] / (outdeg[before] > 0.0 ? outdeg[before] : 1.0);//링크 통해 들어오는 확률 합
                //0으로 나뉘는거 방지

                double new_value = jump + dang_term + d * contrib;//최종 랭크 계산
                pagerank_new_local_chunk[i] = new_value;
                local_l1_diff += fabs(new_value - pagerank_global[cur]);//변화량 절대값
            }
        } else {
            for (long long i = 0; i < locN; ++i) {
                long long cur = off + i;
                ...
                double new_value = jump + dang_term + d * contrib;
                pagerank_new_local_chunk[i] = new_value;
                local_l1_diff += fabs(new_value - pagerank_global[cur]);
            }
        }
         ...
         */
        MPI_Allgatherv(pagerank_new_local_chunk, (int)locN, MPI_DOUBLE, pagerank_global, counts,
