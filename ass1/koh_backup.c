...
void exchange_bound(int rank, struct MPI_info info, double**T_old) {
    ...
    double *message;
    //통신 시작
    if (rank%2==0) {//짝수 먼저 recv 순서 반대로 하우좌상-------------------------------
        for (int i=3;i>=0;i--) {//recv
            ...
            if (rank==0) {
                printf("waiting : origin rank %d count %d dir %d\n",origin_rank,count,3-i);
            }

            MPI_Recv(message, count, MPI_DOUBLE, origin_rank, 3-i, MPI_COMM_WORLD, &st);
            printf("received : rank = %d, count = %d\n", rank, count);
